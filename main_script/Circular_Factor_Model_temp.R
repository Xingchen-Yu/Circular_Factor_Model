...
if(hn==888){
  # load(file='./data/ymat_eu_3.Rdata')
  load(file='./data/euclidean_2d_data.Rdata')
  # ymat = out[[1]]
  print(dim(ymat))
}else{
...
if(continue==F){
  ...
  if(hn==888){
    beta = circular::rvonmises(nr,pi,omega)-pi
  }else{
    beta = rep(0,nr)
...
if(likeli_party==T){
  likeli_gop = likeli_dem = rep(0,iter)
}
...
wrapper_waic_grouped = function(t){
  waic_cpp_grouped(t,nr_par,nc,beta,tau_yes,tau_no,kappa,ymat)
}
...
for(i in 1:iter){
  ...
  if(i %in% seq(start_jitter,iter,skip)){
    ...
    delta2_no = delta_no/2
    #######################################
    ### tuning the proposal variance of the scale parameter \kappa during the initial burnin
    if(i<burnin & continue == F){
  ...
  if(i>burnin){
    ...
    if(WAIC_group==T){
      ...
      # pos_pred = pos_pred + exp(temp2)
      pos_pred2[,j] = temp2
      # pos_pred3 = pos_pred3 + temp2^2
    }else{
  ...
  if(likeli_party==T){
    temp[na+1] = 0
    likeli_dem[i] = sum(temp[dem,])
    likeli_gop[i] = sum(temp[-dem,])
  }
}
...
waic_group = waic_compute_new3(n_pos,pos_pred2)
rm(pos_pred2)
...
# save(file=paste0(h_s,hn,"_beta_start.Rdata"),beta)
# save(file=paste0(h_s,hn,"_tau_yes_start.Rdata"),tau_yes)
# save(file=paste0(h_s,hn,"_tau_no_start.Rdata"),tau_no)
# save(file=paste0(h_s,hn,"_kappa_start.Rdata"),kappa)
# save(file=paste0(h_s,hn,"_ccc_start.Rdata"),ccc)
# save(file=paste0(h_s,hn,"_omega_start.Rdata"),omega)
# save(file=paste0(h_s,hn,"_tsig_start.Rdata"),t_sig)
